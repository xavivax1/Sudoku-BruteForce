...
import static sudoku.chan.SudokuConstants.*;
...
public class Grid implements Cloneable {
    ...
    private int[] cells = new int[MAX_NUMBER_OF_CELLS];
    ...
    // a particular cell.
    private int[] colsSet = new int[UNIT_SIZE];
    // This purpose and behavior of this variable is similar to colsSet.
    private int[] rowsSet = new int[UNIT_SIZE];
    // This purpose and behavior of this variable is similar to colsSet.
    private int[] subgridSet = new int[UNIT_SIZE];
    ...
    // Factory Pattern per crear els grid
    public static Grid create(Reader rd) {
        ...
        try {
            ...
            for (int loc = 0; loc < grid.cells.length; ) {
                ...
                if (ch == COMMENTCHAR) {
                    // Skip to end-of-line
                    while (ch >= 0 && ch != LINEFEED && ch != CARRIAGERETURN) {
                        ch = rd.read();
                    }
                } else if (ch >= '1' && ch <= '9') {
                    // A given
                    grid.set(loc, ch - '0');  // programació implicita !!!
                    loc++;
                } else if (ch == '.' || ch == '0') {
                    // Empty cell
                    loc++;
    ...
    // SRP !!! Fa check i set => 2 funcions
    public boolean set(int cellIndex, int num) {
        // Compute row and column
        int rowIndex = rowIndex(cellIndex, UNIT_SIZE);
        int colIndex = columnIndex(cellIndex);
        int blockLoc = (rowIndex / 3) * 3 + colIndex / 3;

        boolean canSet = cells[cellIndex] == 0
                && (colsSet[colIndex] & (1 << num)) == 0
        ...
        cells[cellIndex] = num;
        colsSet[colIndex] |= (1 << num);
        rowsSet[rowIndex] |= (1 << num);
        subgridSet[blockLoc] |= (1 << num);
        return true;
    }

    private int rowIndex(int cellIndex, int unitSize) {
        return cellIndex / unitSize;
    }
    ...
    private int columnIndex(int loc) {
        return loc % UNIT_SIZE;
    }
